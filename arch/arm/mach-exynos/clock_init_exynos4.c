...
void system_clock_init(void)
{
	...
	/*
	* For CLK_SRC_TOP1, MUX_MPLL_USER_SEL_T  select FINPLL as source
	* SCLKMPLL_USER_T's source is MUX_MPLL_USER_SEL_T
	*/
	...
	writel(CLK_SRC_PERIL0_VAL, &clk->src_peril0);//MUX uart0-4 select SCLKMPLL_USER_T as source
	...
	writel(CLK_DIV_PERIL0_VAL, &clk->div_peril0);//UART0-4 ratio = 7, MPLL=400Mhz, SCLK_UART=100M
	...
	writel(MPLL_CON0_VAL, &clk->mpll_con0);//MPLL 800MHZ,
	...
	//clrsetbits_le32(&clk->src_top1, 1<<12, 1<<12);
}
#endif
